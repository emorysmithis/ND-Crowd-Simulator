...
def create_students(ugrads, grads, cdf, ddf, graph, buildings): 
    # create students lists for each day of the week 
    m_students = []
    t_students = []
    w_students = []
    r_students = []
    ...
    # create file names for each day of the week 
    ...
    for grad in range(ugrads+grads):
        ...
        # put all of the data into a dict 
        m_student = create_dict(sid, speed, -1, m_journey) 
        t_student = create_dict(sid, speed, -1, t_journey) 
        w_student = create_dict(sid, speed, -1, w_journey) 
        r_student = create_dict(sid, speed, -1, r_journey) 
        f_student = create_dict(sid, speed, -1, f_journey) 
        # if there is a journey, add dict to list 
        if m_student: 
            m_students.append(m_student)
        if t_student: 
            t_students.append(t_student)
        if w_student: 
            w_students.append(w_student)
        if r_student: 
            r_students.append(r_student)
        if f_student: 
            f_students.append(f_student)
    # write all lists to their files 
    with open(m_file, 'a') as f: 
        f.write(str(json.dumps(m_students)))
    with open(t_file, 'a') as f: 
        f.write(str(json.dumps(t_students)))
    with open(w_file, 'a') as f: 
        f.write(str(json.dumps(w_students)))
    with open(r_file, 'a') as f: 
        f.write(str(json.dumps(r_students)))
    with open(f_file, 'a') as f: 
        f.write(str(json.dumps(f_students)))

def create_dict(sid, speed, edge_index, journey): 
    if len(journey) > 0: 
        student = {  
                "id": sid,
                "speed": speed,
